...
public class WorkflowConditionEvaluator {
    public boolean evaluate(JsonNode condition, JsonNode variables) {
        if (condition == null || !condition.isObject()) throw new IllegalArgumentException("触发器condition必须是对象");
        ...
        if (actual == null || actual.isMissingNode() || expected == null) return false;
        return switch (operator) {
            ...
            case "!=" -> !equal(actual, expected);
            case "BETWEEN" -> expected.isArray() && expected.size() == 2 && compare(actual, expected.get(0)) >= 0 && compare(actual, expected.get(1)) <= 0;
    ...
    private JsonNode resolve(JsonNode variables, String path) {
        if (variables == null || path == null || path.isBlank()) return null;
    ...
    private boolean contains(JsonNode candidates, JsonNode actual) {
        ...
        for (JsonNode candidate : candidates) if (equal(actual, candidate)) return true;
        return false;
    }
}
