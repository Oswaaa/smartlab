  ...
  <div class="tab-pane-content">
    ...
    <section class="runtime-block">
      ...
      <div class="runtime-list">
        ...
        <div v-if="operationalRegions.length">
          <div v-for="region in operationalRegions" :key="region.regionName" class="runtime-row">
            <span class="runtime-label">功能区域 · {{ region.regionName }}</span>
            <span class="runtime-value">
              <el-tag v-for="state in region.states" :key="region.regionName + state" size="small" type="success" effect="plain">
                {{ state }}
              </el-tag>
        ...
        <div v-else class="runtime-row">
          <span class="runtime-label">功能区域</span>
          <span class="runtime-value muted">设备处于正常运行就绪态</span>
        </div>
        <div v-if="activeExceptionStates.length">
          <div v-for="item in activeExceptionStates" :key="item.regionName + item.state" class="runtime-row">
            <span class="runtime-label">异常区域 · {{ item.regionName }}</span>
            <span class="runtime-value">
              <el-tag size="small" type="danger" effect="plain">{{ item.state }}</el-tag>
              <el-button
                v-if="canControl"
                size="small"
                type="danger"
        ...
        <div v-else class="runtime-row">
          ...
          <span class="runtime-value success-text">
...
.runtime-label { font-size: 12.5px; color: var(--sl-text-secondary); flex-shrink: 0; }
.runtime-value {
  display: flex;
  align-items: center;
...
.runtime-unit { font-size: 12px; font-weight: 500; color: var(--sl-text-secondary); }
...
.success-text { color: var(--sl-success); font-size: 11.5px; font-weight: 600; }
.safe-check-icon { font-size: 14px; color: var(--sl-success); }
.mono { font-family: var(--sl-font-mono); }
.muted { color: var(--sl-text-disabled); font-size: 12px; font-weight: 500; }
.compact-empty { font-size: 12px; color: var(--sl-text-disabled); padding: 12px; }
