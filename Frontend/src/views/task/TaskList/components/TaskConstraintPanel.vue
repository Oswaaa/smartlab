<template>
  <section class="task-constraint-panel">
    ...
    <div v-for="(rule,index) in rules" :key="index" class="constraint-row"><div><strong>{{ rule.ruleName }}</strong><code>{{ rule.expression }}</code><span>{{ Object.keys(rule.bindings||{}).length }} 个变量 · {{ (rule.violationActions||[]).length }} 个动作</span></div><div><el-button class="btn-aliyun-link" link @click="emit('edit', index)">编辑</el-button><el-button class="btn-aliyun-danger-link" link @click="emit('remove', index)">删除</el-button></div></div>
  </section>
</template>
