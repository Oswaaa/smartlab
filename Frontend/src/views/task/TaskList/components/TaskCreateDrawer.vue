<template>
  <el-drawer :model-value="modelValue" title="新建任务" size="78%" class="model-drawer unified-workflow-drawer" destroy-on-close @update:model-value="emit('update:modelValue', $event)">
    ...
    <template #footer><div class="drawer-footer"><el-button @click="emit('update:modelValue', false)">取消</el-button><el-button :loading="preflighting" @click="emit('preflight')">检查</el-button><el-button type="primary" :loading="creating" @click="emit('submit')">创建任务</el-button></div></template>
  </el-drawer>
</template>
