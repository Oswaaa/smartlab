      ...
      <div v-if="task" class="drawer-heading">
        <div class="drawer-identity">
          <div class="drawer-title-row">
            ...
            <el-tag :type="statusType(task.taskStatus)" effect="plain" round>
              {{ statusLabel(task.taskStatus) }}
            </el-tag>
          </div>
          <p>任务 #{{ task.id }} · {{ workflow?.flowName || `流程 ${task.flowModelId}` }} · 开始于 {{ formatTime(task.startTime) }}</p>
        </div>
        <div class="drawer-actions">
          <el-button class="btn-aliyun" :loading="loading" @click="emit('refresh')">刷新</el-button>
          <el-button v-if="task.taskStatus === 'RUNNING'" class="btn-aliyun-danger-link" link @click="emit('terminate')">终止任务</el-button>
        </div>
      </div>
    </template>
...
.drawer-actions .el-button + .el-button { margin-left: 0; }
.execution-drawer { height: 100%; min-height: 0; }
.execution-tabs { height: 100%; display: flex; flex-direction: column; }
.execution-tabs :deep(.el-tabs__header) { flex: 0 0 auto; margin: 0; padding: 0 20px; border-bottom: 1px solid #eceef2; background: #fafbfc; }
.execution-tabs :deep(.el-tabs__nav-wrap::after) { display: none; }
.execution-tabs :deep(.el-tabs__item) { height: 46px; color: #646a73; font-size: 12px; }
.execution-tabs :deep(.el-tabs__item.is-active) { color: #1677ff; font-weight: 600; }
...
.overview-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); margin-bottom: 16px; border: 1px solid #e2e7ed; border-radius: 6px; overflow: hidden; }
.overview-grid > div { display: grid; gap: 5px; padding: 15px 16px; border-right: 1px solid #e2e7ed; background: #fff; }
.overview-grid > div:last-child { border-right: 0; }
.overview-grid span { color: #8a93a6; font-size: 11px; }
.overview-grid strong { color: #1f2329; font-size: 18px; font-weight: 600; }
.task-descriptions { border-radius: 6px; overflow: hidden; }
.graph-layout { height: 100%; min-height: 520px; display: grid; grid-template-columns: minmax(0, 1fr) 360px; border: 1px solid #dfe5ec; border-radius: 6px; overflow: hidden; background: #fff; }
.graph-workspace { min-width: 0; display: flex; flex-direction: column; border-right: 1px solid #dfe5ec; }
.panel-heading { min-height: 48px; display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 8px 12px; border-bottom: 1px solid #e8ebef; background: #fafbfc; }
.panel-heading > div { min-width: 0; display: grid; gap: 2px; }
.panel-heading strong { color: #1f2329; font-size: 12px; }
.panel-heading span, .panel-heading small { color: #8a93a6; font-size: 10px; }
.graph-canvas { flex: 1; min-height: 470px; border: 0; }
.graph-loading { flex: 1; min-height: 470px; padding: 32px; box-sizing: border-box; background: #f8fafc; }
.graph-workspace :deep(.runtime-graph) { height: 100%; min-height: 470px; border: 0; }
.graph-detail { min-width: 0; display: flex; flex-direction: column; background: #fff; }
.graph-detail-scroll, .step-detail-scroll { flex: 1; min-height: 0; overflow: auto; padding: 12px; }
.waiting-detail { display: grid; grid-template-columns: 1fr auto; align-items: start; gap: 8px; padding: 12px; border: 1px solid #e2e7ed; border-radius: 5px; background: #f8fafc; }
.waiting-detail > div { display: grid; gap: 3px; }
.waiting-detail strong { font-size: 12px; }
.waiting-detail span, .waiting-detail p { color: #7b8898; font-size: 10px; }
...
.binding-row { display: grid; grid-template-columns: minmax(0, 1fr) 32px minmax(0, 1fr); align-items: center; gap: 12px; padding: 13px 15px; border: 1px solid #e1e6ec; border-radius: 5px; background: #fff; }
.binding-row > div { display: grid; gap: 3px; }
.binding-row strong { font-size: 12px; }
.binding-row span { color: #7c899a; font-size: 10px; }
.binding-arrow { text-align: center; color: #1677ff !important; font-size: 16px !important; }
.constraint-view section { display: grid; gap: 8px; }
.constraint-view h4 { margin: 8px 0 0; font-size: 12px; }
.rule-row { display: grid; grid-template-columns: 180px minmax(0, 1fr); gap: 10px; padding: 10px 12px; border: 1px solid #e4e8ed; border-radius: 4px; background: #fafbfc; }
.rule-row strong { font-size: 11px; }
.rule-row code { color: #48617e; font-size: 10px; }
...
@media (max-width: 900px) {
  ...
  .graph-workspace { border-right: 0; border-bottom: 1px solid #dfe5ec; }
