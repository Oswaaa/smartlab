<template>
  <section class="data-ports-panel">
    ...
    <el-table v-if="ports.length" :data="ports" class="editable-config-table" table-layout="fixed">
      ...
      <el-table-column label="方向" width="112"><template #default="{ row }"><el-select :model-value="row.direction" :disabled="readonly || isSystemItem(row)" @update:model-value="updatePort(row, 'direction', $event)"><el-option label="IN" value="IN" /><el-option label="OUT" value="OUT" /></el-select></template></el-table-column>
