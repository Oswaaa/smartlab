<template>
  <section class="data-ports-panel">
    <div class="panel-heading"><div><strong>数据端口</strong><span>端口读写绑定的内部变量，紫色连接用于节点间传递数据</span></div><el-button size="small" @click="addPort">新增端口</el-button></div>
    <div v-for="port in ports" :key="port.name || port._systemKey" class="editor-row">
      ...
      <el-button v-else link type="danger" @click="emit('remove-port-request', port.name)">删除</el-button>
    </div>
