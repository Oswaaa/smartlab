<template>
  <section class="variables-panel">
    <div class="panel-heading"><div><strong>变量空间</strong><span>节点内部变量，可供表达式、数据端口和触发器引用</span></div><el-button size="small" @click="addVariable">新增变量</el-button></div>
    <div v-for="variable in variables" :key="variable.name || variable._systemKey" class="editor-row">
      ...
      <el-button v-else link type="danger" @click="deleteVariable(variable.name)">删除</el-button>
    </div>
