...
test('task creation binds devices by clicking nodes in the workflow graph while keeping backend slot ids', () => {
  ...
  assert.doesNotMatch(drawer, /v-if="form\.flowModelId"/)
  const constraints = readSource('views/task/TaskList/components/TaskConstraintPanel.vue')
  const preflight = readSource('views/task/TaskList/components/TaskPreflightPanel.vue')
  assert.doesNotMatch(constraints, /未配置任务级约束/)
  assert.doesNotMatch(preflight, /尚未执行前置检查/)
  assert.match(bindingPanel, /WorkflowStageGraph/)
  const canvasNode = readSource('views/task/WorkflowDesigner/components/WorkflowCanvasNode.vue')
  assert.match(canvasNode, /flex-direction:column/)
  assert.match(canvasNode, /bind-pill[\s\S]*status-pill/)
