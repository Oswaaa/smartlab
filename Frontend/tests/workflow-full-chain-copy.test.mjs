...
describe('Task 9 — Full-Chain Copy Regression', () => {
  ...
  test('task list delegates create flow without losing binding and constraints', () => {
    ...
    assert.match(source, /startTask\(row\.id, 'PRODUCTION'\)/)
    assert.match(source, /startTask\(row\.id, 'SIMULATION'\)/)
    assert.match(source, /仿真执行/)
    assert.match(source, /executionKind/)
    assert.match(source, /模拟/)
    assert.doesNotMatch(drawer, /正式执行|模拟执行|executionKind/)
