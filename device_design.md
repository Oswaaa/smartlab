      ...
      "deviceTemplate":{
        ...
        "commands": [
          { 
            ...
            "description": "设置转速", 
            "parameters": [
              { "name": "targetRpm", "dataType": "INTEGER", "internal": false, "description": "目标转速" }
            ]
          }
        ],
        "events": {
...
####  4：映射关系
- 将模型的属性、命令与 Adapter 契约中的属性、命令进行映射，确保模型的属性、命令能够正确地与 Adapter 交互。

####  5：状态机定义

####  6：内置约束定义

####  7：组件结构清单（COMPONENTS_BOM）
```json
[
  {
    "slotName": "搅拌电机",
...
系统按照槽位的 categoryId 查找：
  → 该类别下有设备模型？
      是 → 展示该组件类别下的所有模型，用户选定模型后展示该模型下的设备实例列表，用户从中选择（可以搜索）
      否 → 提示"该组件类别暂无数字化设备模型，仅记录规格信息"
