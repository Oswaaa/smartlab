"""
Fixture adapter runtime: MQTT registration, heartbeat, telemetry,
...
logger = logging.getLogger("fixture.adapter.runtime")
...
class RuntimeConfig:
    ...
    username: str = ""
    password: str = ""
    client_id: str = "fixture_temp_adapter"
    keepalive: int = 30
    qos: int = 1
    ...
    def load(cls, path):
        ...
        return cls(
            ...
            client_id=sl.get("clientId") or f"fixture_temp_adapter_{uuid.uuid4().hex[:8]}",
...
class AdapterRuntime:
    """Manages MQTT registration, heartbeat, telemetry, events, and command dispatch."""
    ...
    def start(self):
        ...
        # Subscribe AFTER connection is stable (not inside _on_connect callback)
        time.sleep(0.5)
        if self._connected:
    ...
    def _publish_telemetry(self, attributes: dict):
        if not self._connected:
            return
        # Use the first device point (Sensor1)
        device_point = "Sensor1"
    ...
    def _on_message(self, client, userdata, msg):
        try:
            ...
            if cmd == "heat":
                target = float(params.get("targetTemperature", 30))
                hold = int(params.get("holdDurationSec", 5))
                threading.Thread(target=self.core.execute_heat, args=(target, hold, mid), daemon=True).start()
            elif cmd == "cool":
                duration = int(params.get("durationSec", 5))
                threading.Thread(target=self.core.execute_cool, args=(duration, mid), daemon=True).start()
            else:
